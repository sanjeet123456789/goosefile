               ...
               <a role="link" href=" '.$category_post->getparent_file_location().'">' .$category_post->getparent_file_title() .'</a> &raquo;
                <a role="link" style="color:#0397d6;" href="'.$category_post->getpage_id().'">'.$category_post->getTitle().'</a> 
                          ...
                          <button >
                            download
						            ...
						            <div class="realted_post option">';
						            	...
						            	$my_id=$id=urldecode($_GET['id']);
												   $connect = new PDO("mysql:host=localhost;dbname=goosefil_goosefile", "goosefil_pal", "Sitesking@12");
							if ($id >= 0 and $id <= 198){
								$query2 = "SELECT * FROM software_categories where parent_id=1 and  sort_order=1";
							}else if( $id >= 200 and $id <= 298){
								$query2 = "SELECT * FROM software_categories where parent_id=199  and sort_order=1";
							}	
							else if($id >= 300 and $id <= 398)	{
								$query2 = "SELECT * FROM software_categories where parent_id=299 and sort_order=1";
							}	
                    ...
                    foreach($result2 as $row)
                    {
                     ...
                     echo '  
                     <a href="category.php?id='.$row['id'].'" hr> <li><label><div  class="related_category_suggest"  value="'. $row['title'] .'"> ' .$row['title'] .'</div></label>
												    </li></a>';
